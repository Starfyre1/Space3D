/* Copyright (C) Starfyre Enterprises 2024. All rights reserved. */

package com.starfyre1.space3D.core;

import com.starfyre1.space3D.core.entity.Model;
import com.starfyre1.space3D.core.utils.Utils;
import com.starfyre1.space3D.test.Launcher;

import java.nio.FloatBuffer;
import java.util.ArrayList;
import java.util.List;

import org.lwjgl.opengl.GL11;
import org.lwjgl.opengl.GL15;
import org.lwjgl.opengl.GL20;
import org.lwjglx.debug.org.lwjgl.opengl.GL30;

public class ObjectLoader {
	/*****************************************************************************
	 * Constants
	 ****************************************************************************/

	/*****************************************************************************
	 * Member Variables
	 ****************************************************************************/
	private List<Integer>	mVAOS			= new ArrayList<>();
	private List<Integer>	mVBOS			= new ArrayList<>();

	WindowManager			windowManager	= Launcher.getWindowManager();

	/*****************************************************************************
	 * Constructors
	 ****************************************************************************/

	/*****************************************************************************
	 * Methods
	 ****************************************************************************/
	public Model loadModel(float[] vertices) {
		System.out.println(Thread.currentThread().threadId() + " " + windowManager.getWindow());
		int id = createVAO();
		storeDataInAttribList(0, 3, vertices);
		unbind();
		return new Model(id, vertices.length / 3);
	}

	private int createVAO() {
		System.out.println(Thread.currentThread().threadId() + " " + windowManager.getWindow());
		int id = GL30.glGenVertexArrays();
		mVAOS.add(Integer.valueOf(id));
		GL30.glBindVertexArray(id);
		return id;
	}

	private void storeDataInAttribList(int atrrribNo, int vertexCount, float[] data) {
		int vbo = GL15.glGenBuffers();
		mVBOS.add(Integer.valueOf(vbo));
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
		FloatBuffer buffer = Utils.storeDataInFloatBuffer(data);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW);
		GL20.glVertexAttribPointer(atrrribNo, vertexCount, GL11.GL_FLOAT, false, 0, 0);
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, 0);
	}

	private void unbind() {
		GL30.glBindVertexArray(0);
	}

	public void cleanup() {
		for (int vao : mVAOS) {
			GL30.glDeleteVertexArrays(vao);
		}
		for (int vbo : mVBOS) {
			GL15.glDeleteBuffers(vbo);
		}
	}

	/*****************************************************************************
	 * Setter's and Getter's
	 ****************************************************************************/

	/*****************************************************************************
	 * Serialization
	 ****************************************************************************/
}
